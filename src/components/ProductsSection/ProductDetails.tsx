...
const ProductDetails = ({ product, relatedProducts }) => {
  ...
  const handleCommentSubmit = () => {
    if (newComment.trim()) {
      setComments([
        ...comments,
        { user: 'Anonymous', text: newComment, userImage: '/hoodies.jpg' },
      ]);
      setNewComment('');
    }
  };
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto grid gap-8 lg:grid-cols-2">
        ...
        <div className="flex mt-4 gap-2 overflow-x-auto">
          {product.images.map((img, index) => (
            <Image
              ...
              height={100}
              width={100}
              className={`w-20 h-20 rounded-lg cursor-pointer border-2 ${selectedImage === img ? 'border-black' : 'border-transparent'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xl font-semibold text-red-500">${product.discountedPrice}</span>
          ...
          {relatedProducts.map((item, index) => (
            <Card key={index}>
              <CardContent className="p-4">
                <Image src={item.image} alt={item.name} className="w-full h-40 object-cover rounded-lg" width={100} height={100} />
